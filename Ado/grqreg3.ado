*! version 2.3  (17 Apr 2017) Justin Abraham
*  Make functional with graph combine
*! version 2.2  (6 September 2013) Conor Hughes
*  Make default graph region color white
    ...
    if ("`compare'" == "") {;
        ...
        foreach name of varlist `graphvars' {;
            ...
            if ("`ols'" != "") { ;
                ...
                clcolor(black)  legend(off) msize(tiny) )" ;
        ...
        *-> combine all graphs;
        ...
        if ("`nodraw'"=="nodraw") {;
            graph combine `graphlist', graphregion(color(white)) nodraw `options';
        };

        if ("`nodraw'"=="") {;
            graph combine `graphlist', graphregion(color(white)) `options';
        };
