...
foreach yvar in $regvars {
	...
	qui reg `yvar'1 i.spillover##c.`yvar'_sqdev if include == 1 & treat == 0 & ~mi(endlinedate)
	est store e_`yvar'
...
foreach yvar in $regvars {
	...
	loc H1 = "[e_`yvar'_mean]1.spillover#c.`yvar'_sqdev"
	loc H2 = "[e_`yvar'_mean]1.spillover"
	loc H3 = "[e_`yvar'_mean]`yvar'_sqdev"
...
esttab col* using "$output_dir/${reglabel}_sqdev.tex",  cells(none) booktabs nonum nonotes compress replace mtitle("Interaction" "\specialcell{Treated village}" "Sq. distance" "\specialcell{Control mean\\(Std. dev.)}" "Obs.") stats(`statnames', labels(`varlabels') )
