...
*** FWER-ADJUSTED P-VALUES USING STEPDOWN ***
	if "`thisvarlist'" == "indices_ppp" {
		...
		// Stepdown
		set seed 1073741823
		stepdown reg (`thisvarlist1') treat fev*, options(cluster(surveyid)) iter($stepdowniternow) controlstems(_miss0 _full0)
		mat A = r(p)
		stepdown reg (`thisvarlist1') treatXfemalerecXmarried treatXsinglerec spillover fev*, options(cluster(surveyid)) iter($stepdowniternow) controlstems(_miss0 _full0)
		mat B = r(p)
		stepdown reg (`thisvarlist1') treatXmonthlyXsmall treatXlarge spillover fev*, options(cluster(surveyid)) iter($stepdowniternow) controlstems(_miss0 _full0)
		mat C = r(p)
		stepdown reg (`thisvarlist1') treatXlarge spillover fev*, options(cluster(surveyid)) iter($stepdowniternow) controlstems(_miss0 _full0)
		mat D = r(p)
	}
...
*** JOINT ESTIMATION ROW ***
	...
	foreach var in $`thisvarlist' {
		...
		reg `thisvarname' treat fev* `var'_full0 `var'_miss0 $thesecontrol if include == 1 `thisweighting'
		est store spec1_`suestcount'
		local suest1 "`suest1' spec1_`suestcount'"
		reg `thisvarname' treatXfemalerecXmarried treatXsinglerec spillover fev* `var'_full0 `var'_miss0 $thesecontrol if include == 1 `thisweighting'
		est store spec2_`suestcount'
		local suest2 "`suest2' spec2_`suestcount'"
		reg `thisvarname' treatXmonthlyXsmall treatXlarge spillover fev* `var'_full0 `var'_miss0 $thesecontrol if include == 1 `thisweighting'
		est store spec3_`suestcount'
		local suest3 "`suest3' spec3_`suestcount'"
		reg `thisvarname' treatXlarge spillover fev* `var'_full0 `var'_miss0 $thesecontrol if include == 1 `thisweighting'
