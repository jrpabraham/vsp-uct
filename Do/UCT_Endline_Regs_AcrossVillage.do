...
*** FWER-ADJUSTED P-VALUES USING STEPDOWN ***
	if "`thisvarlist'" == "indices_ppp" {
		...
		foreach var in $`thisvarlist' {
			if `hhcase' == 1 & "`var'" ~= "psy_index_z" replace `var'1 = . if maleres
		...
		// Stepdown
		set seed 1073741823
		stepdown reg (`thisvarlist1') treat fev* if purecontrol == 0, options(cluster(surveyid)) iter($stepdowniternow) controlstems(_miss0 _full0)
		mat A = r(p)
		stepdown reg (`thisvarlist1') treat spillover, options(cluster(village)) iter($stepdowniternow)
		mat B = r(p)
		stepdown reg (`thisvarlist1') spillover treat, options(cluster(village)) iter($stepdowniternow)
		mat C = r(p)
	}
...
*** REGRESSIONS FOR EACH ENDLINE OUTCOME ***
	foreach var in $`thisvarlist' {
		...
		*** BASELINE CONTROLS ***
		if "`thisvarlist'" ~= "politicalvars" {
			local baseline_controls "`var'_full0 `var'_miss0"
		}
		else local baseline_controls ""
		...
		*** COLUMN 2: TREATMENT WITHIN VILLAGE ***
		areg `thisvarname' treat `baseline_controls' if ~purecontrol & include == 1 `thisweighting', absorb(village) cluster(surveyid)
		pstar treat
		estadd local thisstat`count' = "`r(bstar)'": col2
		estadd local thisstat`countse' = "`r(sestar)'": col2

		*** COLUMN 3: TREATMENT ACROSS VILLAGE ***
		reg `thisvarname' treat spillover if include == 1 `thisweighting', cluster(village)
		pstar treat
		estadd local thisstat`count' = "`r(bstar)'": col3
		estadd local thisstat`countse' = "`r(sestar)'": col3

		*** COLUMN 4: SPILLOVER EFFECT ***
		pstar spillover
...
*** JOINT ESTIMATION ROW ***
	...
	foreach var in $`thisvarlist' {
		...
		reg `thisvarname' treat `baseline_controls' fev* if ~purecontrol & include == 1 `thisweighting'
		est store spec1_`suestcount'
		local suest1 "`suest1' spec1_`suestcount'"
		reg `thisvarname' treat spillover if include == 1 `thisweighting'
		est store spec2_`suestcount'
		local suest2 "`suest2' spec2_`suestcount'"
		reg `thisvarname' spillover treat if include == 1 `thisweighting'
	...
	esttab col* using "$output_dir/`thisvarlist'_AcrossVillage.tex",  cells(none) booktabs nonotes compress replace alignment(SSSSc) mtitle("\specialcell{Control\\mean (SD)}" "\specialcell{Treatment\\(within villages)}" "\specialcell{Treatment\\(across villages)}" "\specialcell{Spillover\\(thatch HH)}" "N" ) stats(`statnames', labels(`varlabels') )
}
