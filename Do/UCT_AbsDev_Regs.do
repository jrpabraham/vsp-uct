...
*** REGRESSIONS FOR EACH ENDLINE OUTCOME ***
	...
	esttab col* using "$output_dir/`thisvarlist'_absdev.tex",  cells(none) booktabs nonum nonotes compress replace mtitle("Interaction" "Abs. distance" "\specialcell{Treated village}" "\specialcell{Control mean\\(Std. dev.)}" "Obs.") stats(`statnames', labels(`varlabels') )
}
