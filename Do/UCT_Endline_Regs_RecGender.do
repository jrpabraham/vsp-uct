...
*** FWER-ADJUSTED P-VALUES USING STEPDOWN ***
	if "`thisvarlist'" == "indices_ppp" { 
		...
		foreach var in $`thisvarlist' {
			if"`var'" ~= "psy_index_z" replace `var'1 = . if maleres == 1
		...
		// Stepdown
		set seed 1073741823		
		stepdown reg (`thisvarlist1') treatXfemalerecXmarried treatXmalerecXmarried treatXsinglerec spilloverXsingle fev* if ~purecontrol, options(cluster(surveyid)) iter($stepdowniternow) controlstems(_miss0 _full0) 	
		mat A = r(p)
		stepdown reg (`thisvarlist1') treatXmalerecXmarried treatXfemalerecXmarried treatXsinglerec spilloverXsingle fev* if ~purecontrol, options(cluster(surveyid)) iter($stepdowniternow) controlstems(_miss0 _full0) 
		mat B = r(p)
		stepdown reg (`thisvarlist1') treatXfemalerecXmarried treatXsinglerec spillover fev* if ~purecontrol, options(cluster(surveyid)) iter($stepdowniternow) controlstems(_miss0 _full0) 
		mat C = r(p)
		stepdown reg (`thisvarlist1') treatXfemalerecXmarried treatXmalerecXmarried treatXsinglerec spillover purecontrolXsingle, options(cluster(village)) iter($stepdowniternow)
		mat D = r(p)
		stepdown reg (`thisvarlist1') treatXmalerecXmarried treatXfemalerecXmarried treatXsinglerec spillover purecontrolXsingle, options(cluster(village)) iter($stepdowniternow)
		mat E = r(p)
	}
...
*** REGRESSIONS FOR EACH ENDLINE OUTCOME ***
	foreach var in $`thisvarlist' {
		...
		*** COLUMN 2: FEMALE WITHIN VILLAGE ***	
		areg `thisvarname' treatXfemalerecXmarried treatXmalerecXmarried treatXsinglerec spilloverXsingle `var'_full0 `var'_miss0 if ~purecontrol & include == 1 `thisweighting', absorb(village) cluster(surveyid)
		pstar treatXfemalerecXmarried
		estadd local thisstat`count' = "`r(bstar)'": col2
		estadd local thisstat`countse' = "`r(sestar)'": col2
		
		*** COLUMN 3: MALE WITHIN VILLAGE ***
		pstar treatXmalerecXmarried
		estadd local thisstat`count' = "`r(bstar)'": col3
		estadd local thisstat`countse' = "`r(sestar)'": col3
		
		*** COLUMN 4: FEAMLE RECIPIENT ***
		areg `thisvarname' treatXfemalerecXmarried treatXsinglerec spillover `var'_full0 `var'_miss0 if ~purecontrol & include == 1 `thisweighting', absorb(village) cluster(surveyid)		
		pstar treatXfemalerecXmarried
		estadd local thisstat`count' = "`r(bstar)'": col4
		estadd local thisstat`countse' = "`r(sestar)'": col4
		
		*** COLUMN 5: FEMALE ACROSS VILLAGES ***
		reg `thisvarname' treatXfemalerecXmarried treatXmalerecXmarried treatXsinglerec spillover purecontrolXsingle if include == 1 `thisweighting', cluster(village) 
		pstar treatXfemalerecXmarried
		estadd local thisstat`count' = "`r(bstar)'": col5
		estadd local thisstat`countse' = "`r(sestar)'": col5
		
		*** COLUMN 6: MALE ACROSS VILLAGES ***
		pstar treatXmalerecXmarried
...
*** JOINT ESTIMATION ROW ***
	...
	foreach var in $`thisvarlist' {		
		...
		reg `thisvarname' treatXfemalerecXmarried treatXmalerecXmarried treatXsinglerec spilloverXsingle `var'_full0 `var'_miss0 fev* if ~purecontrol & include == 1 `thisweighting'
		...
		reg `thisvarname' treatXfemalerecXmarried treatXsinglerec spillover `var'_full0 `var'_miss0 fev* if ~purecontrol & include == 1 `thisweighting'
		est store spec3_`suestcount'
		local suest3 "`suest3' spec3_`suestcount'"
		reg `thisvarname' treatXfemalerecXmarried treatXmalerecXmarried treatXsinglerec spillover purecontrolXsingle if include == 1 `thisweighting'
	...
	esttab col* using "$output_dir/`thisvarlist'_RecGender_Regs.tex",  cells(none) booktabs nonotes compress replace alignment(SSSSSSc) mtitle("\specialcell{Control\\mean (SD)}" "\specialcell{Female\\recipient\\(within villages)}" "\specialcell{Male\\recipient\\(within villages)}" "\specialcell{Female vs.\\male recipient\\(within villages)}" "\specialcell{Female\\recipient\\(across villages)}" "\specialcell{Male\\recipient\\(across villages)}" "N" ) stats(`statnames', labels(`varlabels') )
}
